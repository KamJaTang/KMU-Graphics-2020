...
void init_buffer_objects()
{
  ...
  for (int i = 0; i < scene->mNumMeshes; ++i)
  {
    ...
    kmuvcl_assimp::Mesh mesh_object;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(mesh->mNormals) * mesh->mNumVertices, &mesh->mNormals[0].x, GL_STATIC_DRAW);

    for (int i = 0; i < mesh->mNumFaces; ++i)
    {
      const aiFace& face = mesh->mFaces[i];

      kmuvcl_assimp::Face face_object;
...
void key_callback(GLFWwindow * window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_L && action == GLFW_PRESS)
    light_position_wc[0] -= 2.0f;
  if (key == GLFW_KEY_R && action == GLFW_PRESS)
    light_position_wc[0] += 2.0f;

  if (key == GLFW_KEY_D && action == GLFW_PRESS)
    light_position_wc[1] -= 2.0f;
  if (key == GLFW_KEY_U && action == GLFW_PRESS)
    light_position_wc[1] += 2.0f;

  if (key == GLFW_KEY_B && action == GLFW_PRESS)
    light_position_wc[2] -= 2.0f;
...
void render_object()
{
  ...
  {
    const kmuvcl_assimp::Mesh& mesh = meshes[i];
    ...
    glUniform4f(loc_u_material_diffuse, material_diffuse[0], material_diffuse[1], material_diffuse[2], material_diffuse[3]);
    ...
    {
      const kmuvcl_assimp::Face& face = mesh.faces[j];
