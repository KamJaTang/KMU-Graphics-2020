...
void init_buffer_objects()
{
  ...
  for (int i = 0; i < scene->mNumMeshes; ++i)
  {
    ...
    mesh_object.material_index = mesh->mMaterialIndex;
  ...
  for (int i = 0; i < scene->mNumMaterials; i++)
  {
    ...
    for (unsigned int j = 0; j < diffuseTextureCount; j++)
    {
          ...
          j, &textureFilePath))
      {
        ...
        std::string path = basepath + textureFilePath.data;
        char path_ch[1000];
        std::strcpy(path_ch, path.c_str());

        // 파일로부터 영상을 이루는 픽셀들을 메인메모리로 로딩
        image = stbi_load(path_ch, &width, &height, &channels, STBI_rgb);

        // TODO: "00_cats.jpg"에 해당하는 텍스쳐 생성, 전송 및 텍스쳐-파라메터 세팅
        glGenTextures(1, &texture_map[textureFilePath.data]);
        glBindTexture(GL_TEXTURE_2D, texture_map[textureFilePath.data]);
...
void draw_mesh(const aiMesh* mesh, const aiMatrix4x4& mat_model)
{
  ...
  for (const kmuvcl::Mesh& mesh_object : meshes)
  {

    glUniform3f(loc_u_light_position_wc, light_position_wc[0], light_position_wc[1], light_position_wc[2]);

    glUniform4f(loc_u_light_ambient, light_ambient[0], light_ambient[1], light_ambient[2], light_ambient[3]);
    glUniform4f(loc_u_light_diffuse, light_diffuse[0], light_diffuse[1], light_diffuse[2], light_diffuse[3]);
    glUniform4f(loc_u_light_specular, light_specular[0], light_specular[1], light_specular[2], light_specular[3]);

    glUniform4f(loc_u_material_ambient, material_ambient[0], material_ambient[1], material_ambient[2], material_ambient[3]);
    glUniform4f(loc_u_material_specular, material_specular[0], material_specular[1], material_specular[2], material_specular[3]);
    ...
    if(mesh_object.has_texture){

      aiMaterial* currentMaterial = scene->mMaterials[mesh_object.material_index];
      // diffuse texture of material
      ...
      {
        aiString textureFilePath;
        if (AI_SUCCESS == scene->mMaterials[mesh_object.material_index]->GetTexture(aiTextureType_DIFFUSE,
            j, &textureFilePath))
        {
          glUniform1i(loc_u_diffuse_texture, j);
    ...
    for (const kmuvcl::Face& face : mesh_object.faces)
    {
